...
import com.corionis.els_plex_generator.repository.Location;
...
public class Main
{
    ...
    private String generateLocal() throws Exception
    {
        ...
        if (repo != null && repo.getLibraryData().libraries.terminal_allowed != null)
            value = repo.getLibraryData().libraries.terminal_allowed.toString();
        else
            value = "true";
        ...
        // add time-out
        if (repo != null)
            value = Integer.toString(repo.getLibraryData().libraries.timeout);
        else
            value = "0";
        json += "\t\t\"timeout\": \"" + value + "\"," + lineSep;
        ...
        // add temp parameters
        if (repo != null)
            value =  repo.getLibraryData().libraries.temp_dated.toString();
        else
            value = "false";
        json += "\t\t\"temp_dated\": \"" + value + "\"," + lineSep;
        if (repo != null)
            value =  repo.getLibraryData().libraries.temp_location;
        else
            value = "output";
        json += "\t\t\"temp_location\": \"" + value + "\"," + lineSep;
    ...
    {
        repo = new Repository(context, Repository.PUBLISHER);
        repo.read(filename, true);
    }
